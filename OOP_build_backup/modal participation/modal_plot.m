function mode_fig=modal_plot(phi_jkn,all_harmonics,state_number,state_name,mode_names,modes_to_plot,colors_to_plot)
...
ylabel(t,'$\phi_n$','Interpreter','latex')
title(t,state_name,'Interpreter','latex','fontsize',14)
t.TileSpacing='compact';
%t.Position=[10 10 700 700];

% plot of all modes indicated in modes_to_plot

nexttile
    for i=1:size(phi_jkn,3)
        phi_to_plot(i) = phi_jkn(state_number,modes_to_plot(1),i);
    end
...
for m=2:length(modes_to_plot)
    ...
    nexttile
    for i=1:size(phi_jkn,3)
        phi_to_plot(i) = phi_jkn(state_number,modes_to_plot(m),i);
    end
